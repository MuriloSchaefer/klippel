...
import { addEdge, addNode, removeEdge, removeNode, updateNode } from "./actions";
...
const slice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(
        ...
        (state: GraphsManagerState, { payload: { graphId, nodeId } }) => {
          ...
          Object.values(graph.edges).forEach((edge) => {
            if (edge.sourceId === nodeId) {
              ...
              graph.adjacencyList[edge.targetId].inputs = graph.adjacencyList[edge.targetId].inputs.filter(c => c !== edge.id);
            }
            if (edge.targetId === nodeId) {
              delete graph.edges[edge.id];
              graph.adjacencyList[edge.sourceId].outputs = graph.adjacencyList[edge.sourceId].outputs.filter(c => c !== edge.id);
            }
          });
      ...
      .addCase(
        updateNode,
        (state: GraphsManagerState, { payload: { graphId, nodeId, changes } }) => {
      ...
      .addCase(
        ...
        (state: GraphsManagerState, { payload: { graphId, edge } }) => {
          ...
          graph.edges[edge.id] = edge;
          graph.adjacencyList[edge.sourceId].outputs.push(
            edge.id
          );
          graph.adjacencyList[edge.targetId].inputs.push(
            edge.id
          );

          graph.nodes[edge.sourceId].outputs[
            edge.targetId
          ] = edge;
          graph.nodes[edge.targetId].inputs[
            edge.sourceId
          ] = edge;
