...
import { LayoutState, ViewportState } from "../state";
...
const getViewportManagerState = (state: {Layout: LayoutState}) => state.Layout.viewportManager
export const selectViewportStates = createSelector(
    getViewportManagerState, 
    (state) => state && state.viewports
    )

export type ViewportStateSelector = <O = ViewportState>(state: ViewportState) => O
export const getViewportState = (name: string, selector?:ViewportStateSelector) => {
