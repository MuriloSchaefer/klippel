...
interface CompositionsManager extends Manager {
  functions: {
    ...
    findComposition(
      filter: (composition: CompositionState) => boolean
  ...
  return {
    functions: {
      ...
      createDebugView(compositionName, viewportName) {
        ...
        const debugVpName = viewportManager.functions.addViewport(
          "Modelo",
          "DebuggerViewport",
          groupName,
          "debug-"
        ...
        dispatch(
          openDebugView({ compositionName, debugViewport: debugVpName })
        );
