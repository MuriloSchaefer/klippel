...
// the node ~300ms (debounced) after the process is added. Opening the audit
// panel before that lands renders the "Tempo não computado" empty state, so
// audit-panel drivers must wait for the row's time to settle first.
export const waitForProcessTimeComputed = async (page: Page, label: string) => {
  await page.waitForFunction(
    (sel: string) => {
      const row = document.querySelector(sel);
      if (!row) return false;
      return !((row as HTMLElement).innerText ?? '').includes(
        'tempo não definido',
      );
    },
    { timeout: 10_000 },
