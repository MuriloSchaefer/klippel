/**
 * E2E test for the renameGarmentShortcut MCP tool. Skips if CDP is unreachable.
 */
...
describe('renameGarmentShortcut (E2E)', () => {
  beforeAll(async () => {
    browser = await puppeteer.connect({ browserURL: CDP_URL, defaultViewport: null });
    const pages = await browser.pages();
    page = pages.find((p) => p.url().startsWith('http://localhost:')) ?? pages[0];
    if (!page) throw new Error('No renderer page found in Electron');
    await resetWorkspace(page, 'e2e-renameGarmentShortcut', 'empty');
  ...
  afterAll(async () => {
    ...
    cleanupWorkspace('e2e-renameGarmentShortcut');
  });
